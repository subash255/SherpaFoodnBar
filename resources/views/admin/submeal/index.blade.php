...
<div class="p-4 bg-white shadow-lg -mt-12 mx-4 z-20 rounded-lg">
    ...
    <!-- Modal Structure -->
    <div id="submealModal" class="fixed inset-0 bg-black bg-opacity-70 modal-hidden items-center justify-center z-50 backdrop-blur-[1px]">
                    ...
                    <tr class="border border-gray-300">
                        ...
                        <td class="border border-gray-300 px-4 py-2">
                            <label for="status{{ $submeal->id }}" class="inline-flex items-center cursor-pointer">
                                <input id="status{{ $submeal->id }}" type="checkbox" class="hidden toggle-switch" data-id="{{ $submeal->id }}" {{ $submeal->status ? 'checked' : '' }} />
                        ...
                        <td class="px-2 py-2 mt-2 flex justify-center space-x-4">
                            <!-- Edit Icon -->
                            <a href="{{ route('admin.submeal.edit', ['id' => $submeal->id]) }}" class="bg-blue-500 hover:bg-blue-700 p-2 w-10 h-10 rounded-full flex items-center justify-center">
                                <i class="ri-edit-box-line text-white"></i>
                            </a>
                            <!-- Delete Icon -->
                            <form action="{{ route('admin.submeal.destroy', ['id' => $submeal->id]) }}" method="post" onsubmit="return confirm('Are you sure you want to delete this submeal?');">
                                @csrf
                                @method('delete')
    ...
    <div class="flex justify-between items-center mt-4">
        <div class="flex items-center space-x-2">
            <span class="ml-4 text-gray-700">
                Showing {{ $submeals->firstItem() }} to {{ $submeals->lastItem() }} of {{ $submeals->total() }} entries
            </span>
        </div>
        <div class="flex items-center space-x-2">
            {{ $submeals->links() }}
